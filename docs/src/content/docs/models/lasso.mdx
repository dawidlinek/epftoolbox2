...
|-----------|------|---------|-------------|
| `predictors` | List | Required | Predictor specifications (strings, `{horizon}` templates, or callables) |
...
Callable predictors accept `(horizon)` or `(horizon, hour)` — see [OLSModel](/epftoolbox2/models/ols/#parameters) for details.
