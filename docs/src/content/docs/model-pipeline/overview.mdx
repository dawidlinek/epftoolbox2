...
<Tabs>
  ...
  <TabItem label="Callables">
    ...
    <Aside>
      Callables are evaluated in the main process before any worker is spawned, so lambdas and closures work without restriction.
    </Aside>
  </TabItem>
...
| `THREADS_PER_PROCESS` | `16` | Threads per process |
