...
`model_index` selects a single model by its 0-based position in the model list. When omitted, all models run. This enables submitting one job per model that allows further pararelization.
