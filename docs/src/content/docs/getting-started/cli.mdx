...
| Flag | Description |
|------|-------------|
| `--model-index N` | Run only `models[N]` — useful for SLURM array jobs |
...
# SLURM array — one model per task
epf run experiment.yaml --model-index $SLURM_ARRAY_TASK_ID
...
<Tabs>
  <TabItem label="Experiment from YAML">
    ```bash
    # 1. Scaffold templates
    epf init

    # 2. Edit the YAML files to suit your experiment
  ...
  <TabItem label="Split data / model runs">
    ```bash
    ...
    # Iterate on model config without re-fetching
    epf model model_pipeline.yaml \
        --data data.csv \
        --test-start 2023-01-01 --test-end 2024-01-01
    ```
  </TabItem>
  <TabItem label="Interactive first run">
    ```bash
    # Build everything interactively, save YAMLs, run
    epf

    # Re-run saved YAMLs without the wizard
    epf run experiment.yaml
    ```
  </TabItem>
  <TabItem label="SLURM array">
    ...
    sbatch --array=0-3 job.sh
    # job.sh runs: epf run experiment.yaml
    ```
  </TabItem>
</Tabs>
