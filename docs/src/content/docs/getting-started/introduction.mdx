...
<CardGrid stagger>
  ...
  <Card title="Workflow Orchestration" icon="seti:config">
    Save full experiments as YAML — data pipeline, model pipeline, run params, and environment settings in one file
  </Card>
</CardGrid>
...
<CardGrid>
  ...
  <Card title="Multiprocessing" icon="random">
    Process pool with inner thread pools - scales across all cores, no <code>__main__</code> guard needed on Linux/Mac
  </Card>
...
<CardGrid>
  ...
  <LinkCard
    title="Data Pipeline"
    description="Fetch, transform, and validate electricity market data"
    href="/epftoolbox2/data-pipeline/overview/"
  />
  <LinkCard
    title="CLI Reference"
    description="Run experiments and build pipelines from the command line"
    href="/epftoolbox2/getting-started/cli/"
  />
</CardGrid>
